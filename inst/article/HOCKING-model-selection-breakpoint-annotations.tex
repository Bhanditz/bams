...
that estimate the generalization error of the trained
smoothing models. The main idea is to use a training set of
annotations to learn the parameter of a smoothing model, then quantify
the error of the model using a test set of annotations. Notably, this
enables quantitative comparison of smoothing models on copy number
...
% Also, from Figure~\ref{figure-kinetics}, it seems that the minimal
% error in these data with the best models is about 5\%, which is
% perhaps annotator-dependent.
...
% Another potential criticism of our annotation-based approach is that
% the results may be biased depending on the annotator.
